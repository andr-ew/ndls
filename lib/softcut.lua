...
local reg = {}
reg.blank = cartographer.divide(cartographer.buffer[1], ndls.zones)
reg.rec = cartographer.subloop(reg.blank)
reg.play = cartographer.subloop(reg.rec)

local voices = ndls.voices
...
local sc = {
    ...
    phase = {
        ...
        set = function(s, n, v)
            s.abs[n] = v
            s[n] = reg.rec:phase_relative(n*2, v, 'fraction')
        end
    },
    fade = function(n, length)
